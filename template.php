...
<body>
	<div class="container">
		...
		<form @submit="submit" method="POST">
			<div v-if="alert" :class="{'alert-success': success, 'alert-danger': fail}" class="alert text-center">{{ alert }}</div>
			<div class="form-group">
				<label for="nameInput">Name</label>
				<input @keyup="resetInfo" class="form-control" name="name" placeholder="allowed: chars, numbers, -, _" id="nameInput">
				<p v-if="errors.nameError" class="alert-danger d-inline-block p-1 mt-2 pl-3 pr-3 rounded">{{ errors.nameError }}</p>
			</div>
			<div class="form-group">
				<label for="phoneInput">Phone</label>
				<input @keyup="resetInfo" class="form-control" type="tel" name="phone" placeholder="+7-xxx-xxx-xx-xx" id="phoneInput">
				<p v-if="errors.phoneError" class="alert-danger d-inline-block p-1 mt-2 pl-3 pr-3 rounded">{{ errors.phoneError }}</p>
			</div>
			<div class="form-group">
				...
				<p v-if="errors.messageError" class="alert-danger d-inline-block p-1 mt-2 pl-3 pr-3 rounded">{{ errors.messageError }}</p>
			</div>
			<div class="form-group">
